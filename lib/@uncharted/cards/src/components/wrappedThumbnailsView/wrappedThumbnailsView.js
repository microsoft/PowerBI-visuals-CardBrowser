        ...
        this.$element.on('scroll', event => {
            ...
            if (!ticking) {
                requestAnimationFrame(() => {
                    this.$element[0].scrollTop + this.$element[0].offsetHeight >= this.$element[0].scrollHeight && this.emit(EVENTS.WRAPPED_THUMBNAILS_VIEW_SCROLL_END, this, event.originalEvent);
                    ticking = false;
                });
                ticking = true;
            }
