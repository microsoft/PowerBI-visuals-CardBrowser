        ...
        this.$element.on('scroll', event => {
            if (!ticking) {
                requestAnimationFrame(() => {
                    this.renderThumbnailsInViewPort();
                    this.$element[0].scrollLeft + this.$element[0].offsetWidth >= this.$element[0].scrollWidth && this.emit(EVENTS.INLINE_THUMBNAILS_VIEW_SCROLL_END, this, event.originalEvent);
                    ticking = false;
                });
                ticking = true;
            }
