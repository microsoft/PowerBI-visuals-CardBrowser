...
export default class Thumbnail extends IBindable {
    ...
    reset(spec = {}) {
        ...
        const imageHeight = (this.data.summary && !this._config['thumbnail.displayLargeImage']) ?
            undefined : this.initialWidth - 10; // thumbnail margins from css TODO: move css to config object
    ...
    _registerDomEvents() {
        ...
        this.$element.on('click', '.meta-data-table a', event => {
            event.stopImmediatePropagation();
            this.emit(EVENTS.THUMBNAIL_CLICK_LINK, event);
            return !this._config['thumbnail.disableLinkNavigation'];
        });
